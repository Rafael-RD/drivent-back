...
import { CreateTicketTypeParams, createEnrollmentWithAddress, createTicket, createTicketType, createTicketTypeConfigurable, createUser } from "../factories";
...
describe('GET /hotels', () => {
    ...
    describe('when token is valid', () => {
        ...
        it('should respond with status 402 when ticket wasnt paid', async () => {
            ...
            const token = await generateValidToken(user);
            const enrollment = await createEnrollmentWithAddress(user);
            const ticketType = await createTicketTypeConfigurable({ hasHotel: true, isRemote: false } as CreateTicketTypeParams);
